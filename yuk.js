...
reply(mess.wait)
let gimg = await(`https://api.zahwazein.xyz/searching/pinterest2?query=${q}&apikey=787520e1bc`)
kreyuk.sendMessage(from, {image:{ url: gimg}, caption: `*Query*: ${q}`}, { quoted: msg })
}
           break
            case 'gimage': {
            if (!q) throw `Example : ${prefix + command} query`
reply(mess.wait)
let gimg = await(`https://api.zahwazein.xyz/searching/gimage2?query=${q}&apikey=787520e1bc`)
kreyuk.sendMessage(from, {image:{ url: gimg}, caption: `*Query: ${q}`}, { quoted: msg })
}
